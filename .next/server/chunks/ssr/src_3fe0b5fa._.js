...
// 1. Enhanced getCroppedImg with better error handling
...
function ProfilePage() {
    ...
    const { user, profile, updateUserProfile, isLoading } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$hooks$2f$useAuth$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useAuth"])();
    ...
    const debugPhotoUpload = (stage, data = {})=>{
        console.log(`🔍 PHOTO DEBUG [${stage}]:`, {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        if (profile) {
            ...
            if (profile.profilePhoto && !croppedImageFile) {
    ...
    // 5. Enhanced file validation in onFileSelect
    const onFileSelect = (e)=>{
        if (e.target.files && e.target.files.length > 0) {
            ...
            // Check for common image formats specifically
            const validTypes = [
                'image/jpeg',
                'image/jpg',
                'image/png',
            ...
            // Validate file size (5MB limit for upload, will be compressed)
            ...
            if (file.size < 1024) {
                toast({
                    ...
                    description: "The image file appears to be corrupted or too small.",
                    variant: "destructive"
                });
                return;
            }
    ...
    const handleCropConfirm = async ()=>{
        debugPhotoUpload('CROP_START', {
            hasCrop: !!completedCrop,
            cropDimensions: completedCrop,
            hasImgRef: !!imgRef.current,
            imageLoaded: imgRef.current?.complete
        });
        if (!completedCrop?.width || !completedCrop?.height || !imgRef.current) {
            debugPhotoUpload('CROP_VALIDATION_FAILED');
            toast({
                title: "Invalid Crop",
        ...
        // Additional validation for minimum crop size
        if (completedCrop.width < 50 || completedCrop.height < 50) {
            toast({
                title: "Crop Too Small",
                description: "Please select a larger area (minimum 50x50 pixels).",
                variant: "destructive"
            });
            return;
        }
        ...
        try {
            debugPhotoUpload('CROP_PROCESSING');
            const croppedFile = await getCroppedImg(imgRef.current, completedCrop, `profile-photo-${user?.uid}-${Date.now()}.jpg`);
            debugPhotoUpload('CROP_RESULT', {
                fileCreated: !!croppedFile,
            ...
            if (croppedFile.size > 10 * 1024 * 1024) {
                throw new Error('The cropped image is too large');
            }
            ...
            setImgSrc(''); // Clear the source image
            debugPhotoUpload('CROP_SUCCESS', {
                previewUrl: newPreviewUrl
            });
            toast({
                title: "Photo Ready",
                description: "Photo cropped successfully. Click 'Save Changes' to upload."
            });
        } catch (error) {
            debugPhotoUpload('CROP_ERROR', {
                error: error.message
            });
    ...
    const onSubmit = async (values)=>{
        debugPhotoUpload('SUBMIT_START', {
            hasPhotoFile: !!croppedImageFile,
        ...
        try {
            ...
            debugPhotoUpload('CALLING_UPDATE_PROFILE', {
                photoFileSize: croppedImageFile?.size,
                updateFields: Object.keys(profileDataToSave)
            });
            await updateUserProfile(profileDataToSave, croppedImageFile);
            debugPhotoUpload('UPDATE_SUCCESS');
            toast({
...
//# sourceMappingURL=src_3fe0b5fa._.js.map