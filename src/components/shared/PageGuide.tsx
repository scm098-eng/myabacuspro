...
export default function PageGuide({ guideKey, triggerLabel = "How to Use", variant = "outline", className }: PageGuideProps) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="w-[95vw] sm:max-w-md rounded-[2.5rem] overflow-hidden border-2 border-primary/20 shadow-2xl animate-in zoom-in-95 max-h-[90vh] flex flex-col p-0">
          ...
          <div className="space-y-4 mt-8">
            {guide.steps.map((step, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/30 border border-muted-foreground/5 animate-in fade-in slide-in-from-left-4" style={{ animationDelay: `${i * 100}ms` }}>
                <div className="flex h-6 w-6 shrink-0 aspect-square items-center justify-center rounded-full bg-primary text-white text-[10px] font-black shadow-sm">
                  {i + 1}
                </div>
