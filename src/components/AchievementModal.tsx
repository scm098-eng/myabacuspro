...
const AchievementModal: React.FC<AchievementProps> = ({ type, studentName, title, score, date, onClose }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.addImage(dataUrl, 'JPEG', 0, 0, 1200, 850);
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300 overflow-y-auto">
      <div className="flex flex-col items-center max-w-5xl w-full my-auto gap-8">
        ...
        <div className="relative w-full flex justify-center items-center overflow-hidden">
           {/* HIDDEN FULL-SIZE CAPTURE CONTAINER */}
           <div 
              ref={cardRef}
                ...
                <div className="w-full flex justify-between items-end mt-4 px-10">
                  ...
                  <div className="text-right space-y-1 w-1/3">
                    <div className="flex flex-col items-end">
                        <div 
                          className="italic text-2xl font-bold text-[#b91c1c] pr-4 mb-[-8px] drop-shadow-sm select-none"
                          style={{ fontFamily: "'Brush Script MT', 'Dancing Script', cursive" }}
                        >
                          Satish Mane
                        </div>
                        <div className="h-0.5 w-40 bg-slate-900 ml-auto mb-2 opacity-20" />
                    </div>
            ...
            {/* SCALED PREVIEW FOR MODAL */}
            <div className="bg-white shadow-2xl relative border-[4px] sm:border-[8px] border-slate-900 overflow-hidden flex flex-col items-center justify-center aspect-[1.411] w-full sm:max-w-3xl rounded-xl">
               <div className="p-4 sm:p-10 flex flex-col items-center text-center justify-between w-full h-full">
                  <div className="flex flex-col items-center scale-50 sm:scale-75 origin-top">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-lg sm:text-2xl font-black uppercase tracking-tight italic text-primary">{config.label}</h3>
                    <p className="text-xl sm:text-4xl font-black uppercase tracking-tighter text-slate-900 px-4">{studentName}</p>
                    <p className="text-xs sm:text-sm font-bold text-slate-500 italic max-w-[80%] mx-auto">{title}</p>
                  </div>
                  <div className="scale-75 sm:scale-100 flex items-center justify-center p-2 rounded-full border-4 border-yellow-100 bg-yellow-50">
                    {config.icon}
                  </div>
               </div>
            </div>
        ...
        <div className="flex flex-wrap gap-3 w-full max-w-2xl px-4 relative z-[10000]">
          <Button 
            onClick={handleDownloadPDF}
            disabled={!!isDownloading}
            className="flex-1 h-14 bg-indigo-600 hover:bg-indigo-700 text-white font-black uppercase tracking-widest rounded-2xl shadow-xl border-none transition-transform hover:scale-[1.02]"
          >
          ...
          <Button 
            ...
            disabled={!!isDownloading}
            className="flex-1 h-14 bg-white text-slate-900 hover:bg-slate-50 font-black uppercase tracking-widest rounded-2xl shadow-lg border-2 border-slate-200 transition-transform hover:scale-[1.02]"
          >
