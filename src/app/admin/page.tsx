...
export default function AdminDashboardPage() {
  ...
  const handleBlastEmails = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const finalMessage = marketingForm.messageMode === 'standard' ? plainTextToHtml(marketingForm.message) : marketingForm.message;
  ...
  return (
    <div className="space-y-8">
            ...
            <Tabs defaultValue="students" className="w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {blogs.map(blog => (
                            <Card key={blog.id}>
                                ...
                                <CardFooter className="text-[10px] text-muted-foreground italic">Last Updated: {normalizeDateDisplay(blog.createdAt)}</CardFooter>
                            </Card>
                        ))}
                    </div>
                </TabsContent>
                ...
                <TabsContent value="moderation">
                    <div className="space-y-8">
                        <Card className="border-orange-200 bg-orange-50/10">
                            <CardHeader><CardTitle className="flex items-center gap-2 text-orange-700"><ShieldAlert className="w-5 h-5" /> Pending Teacher Approvals</CardTitle><CardDescription>Verify credentials before granting platform access.</CardDescription></CardHeader>
                            <CardContent><Table><TableHeader><TableRow><TableHead>Teacher</TableHead><TableHead>Institute</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{processedData.pendingTeachers.length > 0 ? processedData.pendingTeachers.map(t => (<TableRow key={t.uid}><TableCell><div className="font-bold">{t.firstName} {t.surname}</div><div className="text-[10px] text-muted-foreground">{t.email}</div></TableCell><TableCell className="text-sm font-medium">{t.instituteName || 'N/A'}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => handleApproveTeacher(t.uid)}><UserCheck className="w-4 h-4 mr-2" /> Approve</Button><Button asChild variant="ghost" size="sm"><Link href={`/admin/user/${t.uid}`}><Eye className="w-4 h-4" /></Link></Button></div></TableCell></TableRow>)) : <TableRow><TableCell colSpan={3} className="text-center py-8">No pending staff accounts.</TableCell></TableRow>}</TableBody></Table></CardContent>
                        </Card>
                ...
                <TabsContent value="marketing">
                    <Card className="border-primary/10 shadow-lg">
                        <CardHeader className="bg-muted/30 border-b"><CardTitle className="flex items-center gap-2 text-2xl font-headline"><Mail className="w-6 h-6 text-primary" /> Marketing Hub</CardTitle><CardDescription>Blast tailored emails to segments or specific students.</CardDescription></CardHeader>
                        <CardContent className="pt-8">
                            <form onSubmit={handleBlastEmails} className="space-y-8">
                                <div className="space-y-4"><Label className="text-base font-bold">Target Audience</Label><Select value={marketingForm.targetAudience} onValueChange={(val) => setMarketingForm(p => ({ ...p, targetAudience: val }))}><SelectTrigger className="h-14 text-lg border-2 border-primary/20 focus:border-primary"><SelectValue placeholder="Select Audience" /></SelectTrigger><SelectContent><SelectItem value="all">Every Student</SelectItem><SelectItem value="pro">Pro Only</SelectItem><SelectItem value="free">Free Only</SelectItem><SelectItem value="teachers">Teachers Only</SelectItem><SelectItem value="single">Particular Student</SelectItem></SelectContent></Select>{marketingForm.targetAudience === 'single' && (<div className="animate-in fade-in slide-in-from-top-2"><Select value={marketingForm.selectedStudentId} onValueChange={(val) => setMarketingForm(p => ({ ...p, selectedStudentId: val }))}><SelectTrigger className="h-12"><SelectValue placeholder="Pick a student..." /></SelectTrigger><SelectContent>{allUsers.filter(u => u.role === 'student').map(s => (<SelectItem key={s.uid} value={s.uid}>{s.firstName} {s.surname} ({s.email})</SelectItem>))}</SelectContent></Select></div>)}</div>
                                <div className="space-y-2"><Label className="text-base font-bold">Email Subject</Label><Input value={marketingForm.subject} onChange={e => setMarketingForm(p => ({ ...p, subject: e.target.value }))} placeholder="Enter subject line..." className="h-12 bg-muted/20" required /></div>
                                <div className="space-y-4"><div className="flex items-center justify-between"><Label className="text-base font-bold">Email Message</Label><div className="flex bg-muted p-1 rounded-lg"><Button type="button" variant={marketingForm.messageMode === 'standard' ? 'secondary' : 'ghost'} size="sm" className="h-8 rounded-md gap-2" onClick={() => setMarketingForm(p => ({ ...p, messageMode: 'standard' }))}><FileText className="w-3 h-3" /> Standard</Button><Button type="button" variant={marketingForm.messageMode === 'html' ? 'secondary' : 'ghost'} size="sm" className="h-8 rounded-md gap-2" onClick={() => setMarketingForm(p => ({ ...p, messageMode: 'html' }))}><Code className="w-3 h-3" /> HTML Source</Button></div></div><Textarea value={marketingForm.message} onChange={e => setMarketingForm(p => ({ ...p, message: e.target.value }))} rows={10} className="bg-muted/10 border-2 text-lg focus:border-primary" placeholder={marketingForm.messageMode === 'standard' ? "Write your content here. Paragraphs will be added automatically." : "Enter raw HTML content here..."} required /></div>
                                <div className="p-6 bg-muted/30 rounded-2xl border-2 border-dashed border-primary/20 space-y-4"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Paperclip className="w-5 h-5 text-primary" /><span className="font-bold">Attachments</span></div><Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}><Plus className="w-4 h-4 mr-2" /> Add Files</Button><input type="file" multiple ref={fileInputRef} className="hidden" onChange={(e) => { if (e.target.files) { setAttachments(prev => [...prev, ...Array.from(e.target.files!)]); } }}/></div>{attachments.length > 0 && (<div className="flex flex-wrap gap-3">{attachments.map((file, idx) => (<Badge key={idx} className="bg-primary/10 text-primary hover:bg-primary/20 cursor-default px-3 py-1.5 gap-2 border-primary/20"><FileText className="w-3 h-3" />{file.name}<X className="w-3 h-3 cursor-pointer hover:text-destructive" onClick={() => setAttachments(p => p.filter((_, i) => i !== idx))} /></Badge>))}</div>)}</div>
                                <div className="flex items-center gap-6 p-6 bg-primary/5 rounded-2xl border-2 border-primary/10"><div className="flex items-center gap-3"><Checkbox id="test-mode" checked={marketingForm.isTest} onCheckedChange={(val) => setMarketingForm(p => ({ ...p, isTest: !!val }))} /><Label htmlFor="test-mode" className="font-black text-primary uppercase tracking-widest text-xs cursor-pointer">Test Send Only</Label></div>{marketingForm.isTest && (<Input className="flex-1 bg-white border-primary/20" placeholder="Enter recipient for test email..." value={marketingForm.testEmail} onChange={e => setMarketingForm(p => ({ ...p, testEmail: e.target.value }))} />)}</div>
                                <Button type="submit" className="w-full h-16 text-xl font-black uppercase tracking-[0.2em] shadow-xl hover:scale-[1.01] transition-transform" disabled={isResetting !== null}>{isResetting === 'blast' ? <Loader2 className="animate-spin mr-3" /> : <Send className="mr-3 h-6 w-6" />}Launch Marketing Campaign</Button>
                            </form>
                        </CardContent>
                    </Card>
                </TabsContent>
                ...
                <TabsContent value="system">
                    <div className="space-y-8">
                        <Card className="border-primary/20 bg-primary/5"><CardHeader><CardTitle className="flex items-center gap-2"><Settings className="w-5 h-5 text-primary" />System Maintenance</CardTitle><CardDescription>Trigger performance cycles and declare winners manually.</CardDescription></CardHeader><CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="p-6 bg-background rounded-2xl border border-border space-y-4"><div className="flex items-center gap-3"><div className="bg-blue-100 p-2 rounded-lg"><RefreshCw className="w-5 h-5 text-blue-600" /></div><div><h4 className="font-bold">Weekly Cycle</h4><p className="text-xs text-muted-foreground">Monday 00:00 UTC</p></div></div><p className="text-xs text-muted-foreground">Resets weekly points and declares champ.</p><Button onClick={() => handleManualReset('weekly')} variant="outline" className="w-full" disabled={isResetting !== null}>{isResetting === 'weekly' ? <Loader2 className="animate-spin mr-2" /> : <Trophy className="w-4 h-4 mr-2" />}Run Weekly Reset</Button></div><div className="p-6 bg-background rounded-2xl border border-border space-y-4"><div className="flex items-center gap-3"><div className="bg-purple-100 p-2 rounded-lg"><Crown className="w-5 h-5 text-purple-600" /></div><div><h4 className="font-bold">Monthly Cycle</h4><p className="text-xs text-muted-foreground">1st Day 00:00 UTC</p></div></div><p className="text-xs text-muted-foreground">Resets monthly points and notifies students.</p><Button onClick={() => handleManualReset('monthly')} className="w-full bg-purple-600" disabled={isResetting !== null}>{isResetting === 'monthly' ? <Loader2 className="animate-spin mr-2" /> : <Zap className="w-4 h-4 mr-2" />}Declare Monthly Winner</Button></div></CardContent></Card>
                        <Card><CardHeader><CardTitle className="text-lg flex items-center gap-2"><Trophy className="w-5 h-5 text-yellow-500" />Live Winner Status</CardTitle></CardHeader><CardContent><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-2"><Label className="text-[10px] uppercase font-black text-muted-foreground">Weekly Champ</Label><div className="p-4 bg-muted rounded-xl border flex items-center gap-4">{winnersData?.lastWeeklyWinner ? (<><Avatar className="h-10 w-10"><AvatarImage src={winnersData.lastWeeklyWinner.photo}/></Avatar><div><p className="font-bold text-sm">{winnersData.lastWeeklyWinner.name}</p><p className="text-[10px]">{winnersData.lastWeeklyWinner.points} PTS</p></div></>) : <p className="text-xs italic">No weekly winner declared.</p>}</div></div><div className="space-y-2"><Label className="text-[10px] uppercase font-black text-muted-foreground">Monthly Master</Label><div className="p-4 bg-muted rounded-xl border flex items-center gap-4">{winnersData?.lastMonthlyWinner ? (<><Avatar className="h-10 w-10"><AvatarImage src={winnersData.lastMonthlyWinner.photo}/></Avatar><div><p className="font-bold text-sm">{winnersData.lastMonthlyWinner.name}</p><p className="text-[10px]">{winnersData.lastMonthlyWinner.points} PTS</p></div></>) : <p className="text-xs italic">No monthly winner declared.</p>}</div></div></div></CardContent></Card>
                    </div>
                </TabsContent>
            </Tabs>
        </div>
      ...
      <Dialog open={forceWinnerDialog.open} onOpenChange={(val) => !val && setForceWinnerDialog({ open: false, user: null })}>
        <DialogContent className="max-w-md"><DialogHeader><DialogTitle className="flex items-center gap-2"><Trophy className="w-5 h-5 text-yellow-500" /> Declare Champion</DialogTitle><DialogDescription>Name <strong>{forceWinnerDialog.user?.firstName}</strong> as the winner.</DialogDescription></DialogHeader><div className="grid grid-cols-1 gap-4 py-4"><Button onClick={() => handleForceDeclareWinner('weekly')} variant="outline" disabled={isResetting !== null}>Weekly Champion</Button><Button onClick={() => handleForceDeclareWinner('monthly')} variant="outline" disabled={isResetting !== null}>Monthly Master</Button></div><DialogFooter><Button variant="ghost" onClick={() => setForceWinnerDialog({ open: false, user: null })}>Cancel</Button></DialogFooter></DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-5xl max-h-[95vh] overflow-hidden p-0 rounded-[2rem] border-none shadow-2xl flex flex-col">
          <DialogHeader className="p-6 bg-muted/30 border-b shrink-0 flex flex-row items-center justify-between">
            ...
            <div className="flex bg-muted p-1 rounded-xl gap-1">
              <Button variant={blogDialogMode === 'edit' ? 'default' : 'ghost'} size="sm" onClick={() => setBlogDialogMode('edit')} className="rounded-lg h-9 px-4"><Edit className="w-4 h-4 mr-2" /> Editor</Button>
              <Button variant={blogDialogMode === 'preview' ? 'default' : 'ghost'} size="sm" onClick={() => setBlogDialogMode('preview')} className="rounded-lg h-9 px-4"><EyeIcon className="w-4 h-4 mr-2" /> Preview</Button>
            </div>
          </DialogHeader>
            ...
            {blogDialogMode === 'edit' ? (
              <form id="blog-save-form" onSubmit={handleSaveBlog} className="p-8 space-y-10">
                ...
                <div className="bg-orange-50/30 p-8 rounded-3xl border-2 border-orange-100 space-y-8">
                  <div className="flex items-center gap-2 text-orange-700 font-bold uppercase tracking-tight text-xs"><Layout className="w-4 h-4" /> Visual & Layout Engine</div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="space-y-2"><Label className="text-xs font-bold uppercase text-muted-foreground">Article Layout</Label><Select value={editingBlog?.layout || 'standard'} onValueChange={(val: any) => setEditingBlog(p => ({ ...p, layout: val }))}><SelectTrigger className="h-12 border-2 bg-white"><SelectValue placeholder="Pick Layout" /></SelectTrigger><SelectContent><SelectItem value="standard">Standard (Traditional)</SelectItem><SelectItem value="centered">Centered (Focused)</SelectItem><SelectItem value="magazine">Magazine (Wide Hero)</SelectItem><SelectItem value="minimalist">Minimalist (Text Only)</SelectItem><SelectItem value="columns">Two Columns</SelectItem></SelectContent></Select></div>
                    <div className="space-y-2"><Label className="text-xs font-bold uppercase text-muted-foreground">Typography Style</Label><Select value={editingBlog?.fontFamily || 'serif'} onValueChange={(val: any) => setEditingBlog(p => ({ ...p, fontFamily: val }))}><SelectTrigger className="h-12 border-2 bg-white"><SelectValue placeholder="Pick Font" /></SelectTrigger><SelectContent><SelectItem value="serif">Merriweather (Classic Serif)</SelectItem><SelectItem value="sans">Inter (Modern Sans)</SelectItem><SelectItem value="elegant">Lora (Elegant Serif)</SelectItem><SelectItem value="impact">Montserrat (High Impact)</SelectItem></SelectContent></Select></div>
                    <div className="space-y-2"><Label className="text-xs font-bold uppercase text-muted-foreground">Content Spacing</Label><Select value={editingBlog?.lineSpacing || 'relaxed'} onValueChange={(val: any) => setEditingBlog(p => ({ ...p, lineSpacing: val }))}><SelectTrigger className="h-12 border-2 bg-white"><SelectValue placeholder="Pick Spacing" /></SelectTrigger><SelectContent><SelectItem value="tight">Tight (1.4)</SelectItem><SelectItem value="normal">Normal (1.5)</SelectItem><SelectItem value="relaxed">Relaxed (1.6)</SelectItem><SelectItem value="wide">Wide (1.8)</SelectItem></SelectContent></Select></div>
                  </div>
                  <div className="flex items-center gap-3 bg-white/50 p-4 rounded-2xl border-2 border-orange-100/50 w-fit"><Switch id="drop-cap" checked={editingBlog?.dropCap ?? true} onCheckedChange={(val) => setEditingBlog(p => ({ ...p, dropCap: val }))} /><Label htmlFor="drop-cap" className="font-bold cursor-pointer">Enable Large Drop-Cap Feature</Label></div>
                </div>

                <div className="bg-slate-50 p-8 rounded-3xl border-2 border-slate-100 space-y-8">
                  <div className="flex items-center gap-2 text-slate-700 font-bold uppercase tracking-tight text-xs"><Type className="w-4 h-4" /> Headline Identity</div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="space-y-2"><Label className="text-xs font-bold uppercase text-muted-foreground">Boldness</Label><Select value={editingBlog?.headlineWeight || 'black'} onValueChange={(val: any) => setEditingBlog(p => ({ ...p, headlineWeight: val }))}><SelectTrigger className="h-12 border-2 bg-white"><SelectValue placeholder="Pick Weight" /></SelectTrigger><SelectContent><SelectItem value="medium">Medium (500)</SelectItem><SelectItem value="bold">Bold (700)</SelectItem><SelectItem value="black">Extra Black (900)</SelectItem></SelectContent></Select></div>
                    <div className="space-y-2"><Label className="text-xs font-bold uppercase text-muted-foreground">Casing</Label><Select value={editingBlog?.headlineCase || 'normal'} onValueChange={(val: any) => setEditingBlog(p => ({ ...p, headlineCase: val }))}><SelectTrigger className="h-12 border-2 bg-white"><SelectValue placeholder="Pick Casing" /></SelectTrigger><SelectContent><SelectItem value="normal">Sentence Case</SelectItem><SelectItem value="uppercase">All Caps (Impact)</SelectItem></SelectContent></Select></div>
                    <div className="space-y-2"><Label className="text-xs font-bold uppercase text-muted-foreground">Letter Spacing</Label><Select value={editingBlog?.headlineSpacing || 'normal'} onValueChange={(val: any) => setEditingBlog(p => ({ ...p, headlineSpacing: val }))}><SelectTrigger className="h-12 border-2 bg-white"><SelectValue placeholder="Pick Spacing" /></SelectTrigger><SelectContent><SelectItem value="tight">Tight (-0.05em)</SelectItem><SelectItem value="normal">Standard</SelectItem><SelectItem value="wide">Wide (0.1em)</SelectItem></SelectContent></Select></div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2"><Label className="font-bold">Category</Label><Input value={editingBlog?.category || ''} onChange={e => setEditingBlog(prev => ({ ...prev, category: e.target.value }))} required className="h-12 border-2" /></div>
                    <div className="space-y-2"><Label className="font-bold">Author Override</Label><Input value={editingBlog?.author || ''} onChange={e => setEditingBlog(prev => ({ ...prev, author: e.target.value }))} className="h-12 border-2" /></div>
                  </div>
                  <div className="space-y-2"><Label className="font-bold">Excerpt (Social Share Summary)</Label><Textarea value={editingBlog?.excerpt || ''} onChange={e => setEditingBlog(p => ({ ...p, excerpt: e.target.value })) as any} rows={2} required className="border-2" /></div>
                  <div className="space-y-2"><div className="flex items-center justify-between mb-2"><Label className="font-bold">Main Story (HTML Tags Supported)</Label><Badge variant="outline" className="font-bold text-[10px] uppercase">Paragraph Injection ON</Badge></div><Textarea value={draftContent} onChange={e => { setDraftContent(e.target.value); setEditingBlog(prev => ({ ...prev, content: plainTextToHtml(e.target.value) })); }} rows={12} required placeholder="Write your story here..." className="border-2 text-lg leading-relaxed focus:ring-primary font-serif p-6" /></div>
                </div>
              </form>
            ) : (
              <div className="p-8 max-w-4xl mx-auto space-y-12">
                <div className="text-center space-y-4">
                  ...
                  <h1 className={cn("text-4xl md:text-6xl text-slate-900 leading-tight", editingBlog?.headlineWeight === 'black' ? "font-black" : editingBlog?.headlineWeight === 'medium' ? "font-medium" : "font-bold", editingBlog?.headlineCase === 'uppercase' && "uppercase", editingBlog?.headlineSpacing === 'tight' ? "tracking-tighter" : editingBlog?.headlineSpacing === 'wide' ? "tracking-widest" : "tracking-normal")}>{editingBlog?.title || 'Article Headline Preview'}</h1>
          ...
          <DialogFooter className="p-6 border-t bg-muted/10 shrink-0">
            ...
            <Button form="blog-save-form" type="submit" disabled={isSavingBlog} className="h-12 px-10 font-black uppercase tracking-widest shadow-xl transition-transform hover:scale-[1.02]">{isSavingBlog ? <Loader2 className="animate-spin mr-2" /> : <Send className="mr-2 h-4 w-4" />} {editingBlog?.id ? 'Update Article' : 'Publish Article'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
