          ...
          <ScrollArea className="flex-1 p-8 bg-slate-50"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-8">
                {auditResult?.details?.map((detail: any, i: number) => {
                  ...
                  return (<div key={i} className={cn("p-5 rounded-2xl border-2 flex flex-col gap-3 shadow-sm", isSkipped ? "bg-amber-50 border-amber-100" : (isCorrect ? "bg-white border-green-100" : "bg-red-50 border-red-100"))}>
                       ...
                       <div className="mb-2"><p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Question</p><p className="text-sm font-bold text-slate-700 leading-tight">{detail.text || 'Calculation'}</p></div>
                       <div className="flex justify-between items-end pt-2 border-t border-slate-100"><div><p className="text-[9px] font-bold uppercase text-slate-400">Target</p><p className="text-2xl font-black text-slate-900">{detail.correct}</p></div>
